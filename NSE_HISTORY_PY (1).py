...
if __name__ == '__main__':
    ...
    FRDT = int(input("Enter From Dt:"))
    FRM = int(input("Enter From Month:"))
    FRY = int(input("Enter From year(YYYY):"))
    TODT = int(input("Enter to Dt:"))
    TOM = int(input("Enter to Month:"))
    TOY = int(input("Enter tO year(YYYY):"))
    ...
    for i in x:
        symb = i
        df = nse.getHistoricalData(i, SER, date(FRY,FRM,FRDT), date(TOY,TOM,TODT))
        try:
            df.to_excel("D:\\nsepy\\dwn_data\\" + i +".xlsx")

        except AttributeError as e:
            print("An error occurred while trying to save the DataFrame to Excel:", e)
